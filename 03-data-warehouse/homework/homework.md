...
![non-partitioned](./assets/q6_1.png)

![partitioned](./assets/q6_2.png)
...
Because BigQuery already knows the row count.
For native (materialized) BigQuery tables, BigQuery maintains table metadata that includes:
- total number of rows
- column statistics
- storage layout info

COUNT(*) does not need to read any column data at all.

So instead of scanning rows, BigQuery:
- reads the metadata
- returns the precomputed row count
```
